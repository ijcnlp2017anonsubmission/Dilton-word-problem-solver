...
for i in list_question:
    ...
    len_numlist.append(len(numlist))
    answers.append(find_answer(predictedLabels[0],numlist))
total=0
p=0
for j in range(len(answers)):
    print(answers[j],float(equation[j][0]))
    if(str(answers[j])==str(float(equation[j][0]))):
        total=total+1
    elif len_numlist[j]<=2:
        print("\n")
        p=p+1
        print("not added")
        print(list_question[j])
print(total)